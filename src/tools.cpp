...
{
	UKF ukf = car.ukf;
    viewer->addSphere(pcl::PointXYZ(ukf.GetX()[0],ukf.GetX()[1],3.5), 0.5, 0, 1, 0,car.name+"_ukf");
    viewer->addArrow(pcl::PointXYZ(ukf.GetX()[0], ukf.GetX()[1],3.5), pcl::PointXYZ(ukf.GetX()[0]+ukf.GetX()[2]*cos(ukf.GetX()[3]),ukf.GetX()[1]+ukf.GetX()[2]*sin(ukf.GetX()[3]),3.5), 0, 1, 0, car.name+"_ukf_vel");
	if(time > 0)
	{
		...
		{
			ukf.Prediction(dt);
            viewer->addSphere(pcl::PointXYZ(ukf.GetX()[0],ukf.GetX()[1],3.5), 0.5, 0, 1, 0,car.name+"_ukf"+std::to_string(ct));
